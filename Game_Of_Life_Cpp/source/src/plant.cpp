...
{
    this->get_type() = OrganismType::Plant;
...
void Plant::action(char** board, int height, int width)
{
    ...
    if (name == "Grass" || name == "Guarana" || name == "Belladonna")
        default_action();
    else if (name == "Sow_thistle")
        sow_thistle_action();
    else
        sosnowsky_hogweed_action();
}
...
void Plant::sosnowsky_hogweed_action()
{
...
void Plant::collision(char** board, std::vector<Organism*>& organisms, std::vector<int>& indexes_to_remove, std::vector<PlaceToAddOrganism>& added_plants)
{
    ...
    if (can_sow)
    {
        ...
        if (counter > 5)
            ...
            added_plants.push_back({OrganismType::Plant, this->get_name(), this->get_position_row(), this->get_position_column()});
    }
    else
